...
This work aims to develop a formal, quantitative framework
for measuring metadata de-anonymization risk in token-based
CBDC systems. This objective is operationalised through the
following five concrete aims:

\begin{enumerate}
\item Derive a normalised privacy risk metric $R_{\mathrm{norm}}$
from mathematical first principles, bounded in $[0, 1]$ with a
proven theoretical upper bound.

\item Model six cryptographic schemes spanning the full privacy
spectrum, quantifying how each scheme transforms the value
domain of observable metadata attributes.

\item Incorporate Nepal-specific metadata attributes into the
privacy risk model.

\item Conduct empirical analysis across eight experimental
conditions including scheme comparison, population scalability,
mitigation effectiveness, and attribute sensitivity.

\item Translate quantitative findings into policy recommendations
for Nepal Rastra Bank to guide CBDC cryptographic scheme
selection.
\end{enumerate}
