...
\begin{itemize}
    ...
    effectively continuous. Under Zero-Knowledge Proof Token, $V_i$ is a
...
scheme then transforms what is actually observable: zero-knowledge proof range
...
The adversary is modeled as a \textit{passive,
...
work and represent a separate research problem. The
...
\cite{sweeney2002}. A user satisfies k-anonymity
...
\midrule
User1 & Kathmandu valley & No \\
User2 & Hill-Rural         & Yes \\
User3 & Kathmandu valley   & No \\
User4 & Terai-Urban        & Yes \\
User5 & Kathmandu valley   & No \\
\bottomrule
\end{tabular}
\end{table}
...
User2 and User4 are uniquely identifiable from zone alone.
User1, User3, and User5 are protected by sharing the
...
unique, they are fully identifiable from the pair
...
Narayanan and Shmatikoff \cite{narayanan2008} in the
...
\textbf{Pair} & \textbf{Unique?} \\
\midrule
User1 & Kathmandu       & Mobile & (Kathmandu, Mobile)   & No \\
User2 & Hill-Rural     & Mobile & (Hill-Rural, Mobile)  & Yes \\
User3 & Kathmandu       & QR     & (Kathmandu, QR)        & Yes \\
User4 & Terai-Urban    & Mobile & (Terai-Urban, Mobile) & Yes \\
User5 & Kathmandu       & Mobile & (Kathmandu, Mobile)   & No \\
\bottomrule
\end{tabular}
\end{table}

From the earlier example, $s(\text{zone}) = 0.400$,
$s(\text{channel}) = 1/5 = 0.200$ (User3 is the only
QR user).

$s(\text{zone, channel}) = 3/5 = 0.600$
(User2, User3, User4 are jointly unique).
...
independence predicts. User3 was protected on zone alone
...
For this work with $m = 7$ attributes, the number of
distinct pairs is,
...
In the results of this work, most pair terms contribute
...
(3.8) simultaneously reaches its
...
From (3.9), each pair term equals
...
Using the original worked example from the work
...
in the high category. Zero-Knowledge Proof Token with
...
A multiplier of $\mu_i = 1.0$ means no mitigation,the attribute is observed at full precision. A multiplierof $\mu_i = 0.0$ means perfect suppression,
...
Androulaki et al. \cite{androulaki2018} describe Fabric's
...
not arbitrary, it reflects what a passive observer with read
...
problem addressed in this work. As argued in
...
settlement rails and are excluded. The seven modeled
...
This work models seven metadata attributes, five standard
...
attribute but excluded on scope grounds. This work models
...
All $\Rnorm$ values in this work are normalised against
this ceiling of 28.
...
Six cryptographic schemes are modeled, spanning the full
...
\cite{chaum1983} blinding protocol. The bank signs a
...
signature construction \cite{vansaberhagen2013}. The
...
\textbf{Zero-Knowledge Proof Token} applies zero-knowledge range proofs
\cite{groth2015} to reduce all attributes to coarse
...
This work uses synthetically generated transaction data
...
Appendix~\ref{app:a_code}. All analyses in
